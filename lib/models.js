...
function trainModels(res, sequences, counter)
{
	...
	for (let i = 0; i < counter.length; i++)
		models[counter[i]['key']] = new HMM(1500);
	for (let i = 0; i < counter.length; i++)
	{
	...
	for (let j = 0; j < sequenceId.length; j++)
	{
		...
		for (let i = 0; i < Object.keys(models).length; i++)
		{
			...
			for (let k = j + 2; k < sequenceId.length+1; k++)
			{
				...
				if (j > 0)
					currentProb1 = models[Object.keys(models)[i]].forward(sequenceId.slice(j-1, k)).prob;
				currentProb2 = models[Object.keys(models)[i]].forward(sequenceId.slice(j+1, k)).prob;
				if (k == sequenceId.length)
				{
					phoneme.push({'model': Object.keys(models)[i], 'k': k, 'prob': currentProb});
					break;
				}
				if (max < currentProb || max < currentProb1 || max < currentProb2)
				{
					if (currentProb > max)
						max = currentProb;
					if (currentProb1 > max)
