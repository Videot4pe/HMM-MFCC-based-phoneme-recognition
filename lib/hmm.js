...
module.exports = class HMM {
	constructor(n)
	{
		this.states = ['start', 'mid', 'end'];
		//this.endState = 'end';
		this.obs = [...Array(n).keys()];
		this.initProbs = {'start': 0.8, 'mid': 0.1, 'end': 0.1};
		this.transitionProbs = {'start': {'start': 0.1, 'mid': 0.8, 'end': 0.1}, 'mid': {'start': 0, 'mid': 0.7, 'end': 0.3}, 'end': {'start': 0, 'mid': 0.1, 'end': 0.9}};
	...
	forward(obs)
	{
		let steps = [];
		let previousStep = {};
		let currentStep = {'start': 0, 'mid': 0, 'end': 0};
		let alpha = 0;
		for (let i = 0; i < obs.length; i++)
		{
			currentStep = {'start': 0, 'mid': 0, 'end': 0};
	...
	baumWelch(obs)
	{
		...
		// 	{
		// 		//console.log('emis: ',this.emissionProbs[obs[i]]);
		// 		this.emissionProbs[obs[i]] = fill();
		// 		//console.log('emis after: ',this.emissionProbs[obs[i]]);
		// 	}
		//console.log('emis: ', this.emissionProbs);
		...
		//console.log('init before: ', this.initProbs);
		//console.log(gamma);
		for (let i of this.states)
			this.initProbs[i] = gamma[0][i];
		//console.log('init after: ', this.initProbs);

		//console.log('transitionProbs before: ', this.transitionProbs);
		for (let i of this.states)
		{
			let sum = 0;
			for (let j of this.states)
		...
		//console.log('transitionProbs after: ', this.transitionProbs);

		//console.log('emissionProbs before: ', this.emissionProbs);
		for (let i of this.obs)
		{
			...
			for (let j of this.states)
			{
				this.emissionProbs[i][this.states.indexOf(j)] = sumGammaT(gamma, j, this.emissionProbs, this.states)/sumGamma(gamma, j);
				sum += this.emissionProbs[i][this.states.indexOf(j)];
			}
			for (let j of this.states)
				this.emissionProbs[i][this.states.indexOf(j)] = 1 / sum * this.emissionProbs[i][this.states.indexOf(j)];
		}
		//console.log('emissionProbs after: ', this.emissionProbs);
	}
...
function sumGammaT(gamma, j, emission, states)
{
	let sum = 0;
	for (let i = 0; i < gamma.length; i++)
		sum += gamma[i][j] * emission[i][states.indexOf(j)];
	return sum;
}
